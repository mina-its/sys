...
function isProductionMode() {
    return process.env.NODE_ENV === types_1.EnvMode.Production;
}
...
async function putSub(cn, objectName, itemID, property, subItem) {
    let collection = await getCollection(cn, objectName);
    let command = { $addToSet: {} };
    command.$addToSet[property] = subItem;
    await collection.updateOne({ _id: itemID }, command);
}
exports.putSub = putSub;
async function deleteSub(cn, objectName, itemID, property, subItemID) {
    let collection = await getCollection(cn, objectName);
    let command = { $pull: {} };
...
async function patchSub(cn, objectName, itemID, property, subItemID, subItem) {
    let collection = await getCollection(cn, objectName);
    let command = { $set: {} };
    command.$set[`${property}.$[elem]`] = subItem;
    await collection.updateOne({ _id: itemID }, command, { arrayFilters: [{ "elem._id": subItemID }] });
}
exports.patchSub = patchSub;
...
async function putFile(cn, drive, relativePath, file) {
    switch (drive.type) {
        case types_1.SourceType.File:
...
async function loadDrives() {
    exports.glob.drives = [];
    for (const db of exports.glob.services) {
        let drives = await get(db, types_1.Objects.drives);
        for (const drive of drives) {
            drive._ = { db };
            drive.entityType = types_1.EntityType.Drive;
            switch (drive.type) {
                case types_1.SourceType.File:
            ...
            exports.glob.entities.push(drive);
...
function initializeRolePermissions(justEntity) {
    for (const role of exports.glob.roles) {
        ...
        for (let permit of role.permissions) {
            ...
            let permission = (permit.objAction || permit.funcAction || permit.formAction || permit.driveAction);
...
function toPublicUrl(cn, uri, addPrefix = false) {
    uri = _.trim(uri, '/');
